      ...
      <nav class="flex w-full max-w-7xl items-center justify-between py-2">
        ...
        <div class="flex items-center text-base leading-5">
          <div class="flex flex-row items-center">
            <NuxtLink class="px-3 py-2 text-gray-700 transition hover:text-emerald-500" to="/articles">
              文章
            </NuxtLink>
            <ClientOnly>
              <NavigationBarAvatarMenu v-if="userProfile.id" />
              <NuxtLink v-else class="px-3 py-2 text-gray-700 transition hover:text-emerald-500" to="/login">
                登入
              </NuxtLink>
            </ClientOnly>
          </div>
